...
type Task = { id: string; title: string; scenario: string; seedQuery: string; evidenceSeed: string; fixedFirstQuery: boolean };
...
type SearchAttempt = { ids: string[]; mode: SearchResult["mode"] };
...
export default function RecallLab() {
  ...
  const [searchHistory, setSearchHistory] = useState<SearchAttempt[]>([]);
  ...
  const rankHistory = evaluation ? searchHistory.map((attempt, index) => ({
    number: index + 1,
    rank: attempt.ids.indexOf(evaluation.targetId) + 1,
    mode: attempt.mode,
  })) : [];
  ...
  async function search(phrase: string) {
    ...
    if (attempts >= 10) { setMessage("This test allows up to ten searches. Choose a photo or stop the task."); return; }
    setBusy(true);
    setMessage("");
    ...
    try {
      ...
      setResult(payload);
      setSearchHistory((history) => [...history, { ids: payload.ids, mode: payload.mode }]);
  ...
  async function submitAnonymousResult() {
    ...
    try {
      const response = await fetch("/api/recall-lab/evaluate", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ taskId: task.id, selectedPhotoId, record: true, sessionId, mode, attempts, elapsedMs, topFiveIds: result.ids.slice(0, 5), aiMode: result.mode, rankedIdsByAttempt: searchHistory.map((item) => item.ids), attemptModes: searchHistory.map((item) => item.mode) }),
      });
  ...
  return <div className="lab-shell">
    ...
    <main className="lab-main">
      <section className="lab-intro"><span className="lab-eyebrow">Test the proposed retrieval experience</span><h1>Can one better clue help you find the right photo?</h1><p>Try a scripted task in a 27-photo synthetic library. This tests the journey after a broad first search—not the real Google Photos search engine or your personal library.</p><div className="lab-method"><strong>What is real?</strong> Three task patterns came from anonymous survey responses R01, R08, and R07. <strong>What is invented?</strong> Every image and scene is synthetic; the café task is a test-only stress case, not another respondent.</div></section>
      ...
      {catalog && !task && <section className="lab-setup"><div className="lab-section-heading"><span>Step 1</span><h2>Choose a retrieval task</h2><p>Try a broad clue, a look-alike result set, or the controlled café case where the first clue buries the target.</p></div><div className="lab-task-grid">{catalog.tasks.map((item, index) => <button key={item.id} className="lab-task" onClick={() => chooseTask(item)}><span className="lab-task-number">0{index + 1}</span><strong>{item.title}</strong><p>{item.scenario}</p><small>{item.evidenceSeed}</small><span className="lab-task-link">Start task →</span></button>)}</div><p className="lab-corpus-note">{catalog.note}</p></section>}
      {catalog && task && <>
        ...
        <section className="lab-context"><div><span className="lab-eyebrow">Your retrieval mission</span><h2>{task.scenario}</h2><p>{task.fixedFirstQuery ? "The first broad query is fixed for this controlled benchmark. After that, add a detail you remember." : "Start with the broad clue shown below, then use what else you remember if the results are weak."}</p></div><div className="lab-source-note">{task.evidenceSeed}. The scene details are synthetic.</div></section>
        <section className="lab-search-area"><div className="lab-section-heading"><span>Step 2</span><h2>Try the first search</h2></div><div className="lab-mode" role="group" aria-label="Search experience"><button className={mode === "guided" ? "active" : ""} onClick={() => { if (!result) setMode("guided"); }} disabled={Boolean(result)}>Guided with AI</button><button className={mode === "baseline" ? "active" : ""} onClick={() => { if (!result) setMode("baseline"); }} disabled={Boolean(result)}>Basic keyword search</button></div><p className="lab-mode-note">{mode === "guided" ? "AI ranks the sample photos from your clue. If it cannot respond, the screen will say so and use basic matching." : "Basic matching uses only words in the sample photo descriptions. No AI guidance is provided."}</p><form className="lab-query-form" onSubmit={(event) => { event.preventDefault(); void search(query); }}><label htmlFor="lab-query">What do you remember?</label><div><input id="lab-query" value={query} onChange={(event) => setQuery(event.target.value)} minLength={2} maxLength={160} readOnly={task.fixedFirstQuery && attempts === 0} disabled={busy || Boolean(evaluation) || attempts >= 10} aria-describedby="lab-query-help"/><button type="submit" disabled={busy || Boolean(evaluation) || attempts >= 10 || query.trim().length < 2}>{busy ? "Working…" : attempts ? "Search again" : "Find photos"}</button></div><small id="lab-query-help">{task.fixedFirstQuery && attempts === 0 ? "The first query is fixed for this controlled test. You can edit it after the results appear." : "The suggested clue is broad. Add or change words as needed."}</small></form>{message && !evaluation && <p className="lab-message" role="status">{message}</p>}</section>
        ...
        {evaluation && <section className="lab-finish"><span className="lab-eyebrow">Task result</span><h2>{evaluation.correct ? "You found the intended photo" : "This was not the intended photo"}</h2><p>{evaluation.correct ? "That is the target for this synthetic task." : selectedPhotoId ? `You chose ${selectedPhotoId}. The target was ${evaluation.targetId}.` : `You stopped. The target was ${evaluation.targetId}.`}</p>{targetPhoto && <div className="lab-target"><div className="lab-photo-image" role="img" aria-label={targetPhoto.alt} style={photoStyle(targetPhoto)}/><span>Target: {targetPhoto.alt}</span></div>}<div className="lab-result-stats"><span><strong>{attempts}</strong> search {attempts === 1 ? "attempt" : "attempts"}</span><span><strong>{(elapsedMs / 1000).toFixed(1)}s</strong> elapsed</span><span><strong>{result?.ids.slice(0, 5).includes(evaluation.targetId) ? "Yes" : "No"}</strong> target in last top 5</span></div><div className="lab-rank-history"><h3>Where was the target after each search?</h3><p>Shown only after the task ends. Every catalog photo is ranked, so this measures position—not whether Google Photos indexed a real image.</p><ol>{rankHistory.map((item) => <li key={item.number}><strong>Attempt {item.number}</strong><span>Rank {item.rank} of {catalog.photos.length} · {item.rank <= 5 ? "in the top 5" : item.rank <= 12 ? "below the top 5" : "hidden until Show all"}</span><small>{item.mode === "groq" ? "Groq AI" : item.mode === "keyword_fallback" ? "keyword fallback" : "basic keyword"}</small></li>)}</ol></div><p className="lab-privacy">Optional: share task ID, search mode, attempts, time, selected synthetic photo ID, success, and target rank after each search. No name, account, personal photo, or search words are saved.</p><button className="lab-primary" onClick={() => void submitAnonymousResult()} disabled={busy || recorded}>{recorded ? "Anonymous result saved" : "Share anonymous test result"}</button><button className="lab-secondary" onClick={() => { setTaskId(""); setResult(null); setEvaluation(null); }}>Try another task</button>{message && <p className="lab-message" role="status">{message}</p>}</section>}
      </>}
    </main>
