...
type LibraryView = "sources" | "stories" | "survey";
...
type PrimaryResearch = {
  study: {
    title: string; instrumentUrl: string; sampleType: string; privacyNote: string;
    responseCount: number; currentUserCount: number; recentEpisodeCount: number;
    effortOrFailureCount: number; caveat: string;
  };
  patterns: {
    targetTypes: Array<{ code: string; label: string; count: number }>;
    remembered: Array<{ code: string; label: string; count: number }>;
    forgotten: Array<{ code: string; label: string; count: number }>;
    firstMethods: Array<{ code: string; label: string; count: number }>;
    firstResults: Array<{ code: string; label: string; count: number }>;
    nextActions: Array<{ code: string; label: string; count: number }>;
    finalOutcomes: Array<{ code: string; label: string; count: number }>;
  };
  episodes: Array<{
    id: string; targetType: string; targetSummary: string | null; remembered: string[]; forgotten: string[];
    firstMethod: string; exactQuery: string | null; firstResult: string; nextActions: string[];
    finalOutcome: string; qualityFlags: string[];
  }>;
};
...
type GroundedAnalysis = {
  ...
  provenance: { includedStories: number; cacheStatus: "hit" | "generated"; synthesisMode?: "groq" | "deterministic_fallback" };
};
...
type SystemState = { kind: "loading" } | { kind: "offline" } | {
  ...
  opportunity: OpportunityMap; problem: ProblemDefinition; primary: PrimaryResearch;
};
...
const sourceMethods: Record<string, { name: string; method: string; api: string; note: string }> = {
  ...
  forum: { name: "Other public forums", method: "Verified public excerpt import", api: "No key required", note: "Google-Photos-specific discussions are source-checked, imported with their original links, and screened by the same inclusion rule." },
};
    ...
    async function loadSystem() {
      try {
        const responses = await Promise.all([
          ...
          request("/api/opportunity-map"), request("/api/problem-definition"), request("/api/primary-research"),
        ]);
        if (!responses.every((response) => response.ok)) throw new Error("System check failed");
        const [health, stats, evidence, research, sourceCoverage, opportunity, problem, primary] = await Promise.all(responses.map((response) => response.json())) as [
          ...
          { data?: Evidence[] }, ResearchQuestions, SourceCoverage, OpportunityMap, ProblemDefinition, PrimaryResearch,
        ];
        ...
        setSystem({
          ...
          evidence: evidence.data ?? [], bySource: stats.bySource ?? [], research, sourceCoverage, opportunity, problem, primary,
        });
      } catch (error) {
  ...
  return <div className="app-shell-v3">
    ...
    <main className="workspace-v3">
      ...
      {system.kind === "online" && view === "overview" && <section className="page-v3">
        ...
        <section className="survey-pulse-v3"><div className="survey-pulse-head-v3"><div><span className="section-label-v3">Primary research · new</span><h2>Real users add a clue the public stories missed</h2><p>Among {system.primary.study.recentEpisodeCount} recent retrieval attempts, <strong>{system.primary.patterns.forgotten.find((item) => item.code === "album_or_folder")?.count ?? 0}</strong> respondents forgot the album or folder—more than forgot the date or place.</p></div><button className="secondary-v3" onClick={() => { setLibraryView("survey"); navigate("library"); }}>Explore survey evidence →</button></div><div className="survey-pulse-grid-v3"><div className="survey-numbers-v3"><article><strong>{system.primary.study.responseCount}</strong><span>anonymous responses</span></article><article><strong>{system.primary.study.recentEpisodeCount}</strong><span>recent retrieval attempts</span></article><article><strong>{system.primary.study.effortOrFailureCount}</strong><span>reported effort or failure</span></article><article><strong>{system.primary.patterns.firstResults.filter((item) => item.code !== "exact_photo").reduce((sum,item) => sum + item.count,0)}</strong><span>first-result problems</span></article></div><div className="survey-bars-v3"><h3>What they remembered</h3>{system.primary.patterns.remembered.slice(0,4).map((item)=><div key={item.code}><span>{item.label}</span><i><b style={{width:`${percent(item.count,system.primary.study.recentEpisodeCount)}%`}}/></i><strong>{item.count}/{system.primary.study.recentEpisodeCount}</strong></div>)}</div><div className="survey-bars-v3 forgotten"><h3>What they explicitly forgot</h3>{system.primary.patterns.forgotten.slice(0,4).map((item)=><div key={item.code}><span>{item.label}</span><i><b style={{width:`${percent(item.count,system.primary.study.recentEpisodeCount)}%`}}/></i><strong>{item.count}/{system.primary.study.recentEpisodeCount}</strong></div>)}</div></div><footer>{system.primary.study.caveat}</footer></section>

        <div className="overview-grid-v3">
          <article className="panel-v3 chart-panel-v3"><div className="panel-head-v3"><div><span className="section-label-v3">Where retrieval breaks</span><h2>Five problems, compared in one view</h2></div><button onClick={() => navigate("compare")}>Open comparison →</button></div><div className="chart-layout-v3"><div className="donut-v3 large" style={{background: opportunityDonut} as CSSProperties}><span><strong>{opportunityTotal}</strong><small>classified<br/>breakdowns</small></span></div><div className="legend-v3">{system.opportunity.areas.map((area) => <button key={area.code} onClick={() => openOpportunity(area.code)}><i style={{background: opportunityColors[area.code]}}/><span>{breakdownLabels[area.code]}</span><strong>{percent(area.episodes, opportunityTotal)}%</strong><small>{area.episodes} {area.episodes === 1 ? "story" : "stories"}</small></button>)}</div></div><p className="caveat-v3">Percentages describe {corpus.evidence_count} selected public stories. They do not estimate how common each problem is among all Google Photos users.</p></article>
      ...
      {system.kind === "online" && view === "library" && <section className="page-v3">
        ...
        <div className="toggle-v3"><button className={libraryView === "sources" ? "active" : ""} onClick={() => setLibraryView("sources")}>How data gets here</button><button className={libraryView === "stories" ? "active" : ""} onClick={() => setLibraryView("stories")}>Read {corpus.evidence_count} public stories</button><button className={libraryView === "survey" ? "active" : ""} onClick={() => setLibraryView("survey")}>Primary research · {system.primary.study.responseCount}</button></div>
        {libraryView === "sources" && <><section className="pipeline-panel-v3"><div className="panel-head-v3"><div><span className="section-label-v3">The evidence pipeline</span><h2>AI organizes evidence. It does not invent the evidence.</h2></div></div><div className="pipeline-v3"><div><span>1</span><i>◎</i><strong>Public conversation</strong><small>Review, post, comment, or support thread</small></div><b>→</b><div><span>2</span><i>⇩</i><strong>Collector or import</strong><small>Original text, URL, platform, and date</small></div><b>→</b><div><span>3</span><i>✦</i><strong>Groq structures it</strong><small>Memory clues, behavior, and failure point</small></div><b>→</b><div><span>4</span><i>✓</i><strong>Researcher checks it</strong><small>Compared with the original public source</small></div><b>→</b><div><span>5</span><i>▥</i><strong>D1 powers insights</strong><small>Counts use database rules, not AI guesses</small></div></div></section><section className="quality-v3"><div className="quality-title-v3"><span className="section-label-v3">Research quality</span><h2>What becomes evidence—and what does not</h2><p>The engine keeps collection, AI interpretation, and human judgement separate so every conclusion can be challenged.</p></div><div className="quality-rules-v3"><article><span>Included</span><strong>A specific remembered photo or video</strong><p>The person describes a target, at least one remembered clue, and a retrieval attempt or barrier.</p></article><article><span>Excluded</span><strong>Generic complaints and recovery issues</strong><p>Sentiment, deletion, backup, sync, and vague “search is bad” comments do not count as retrieval stories.</p></article><article><span>Safe conclusion</span><strong>Compare mechanisms, not market size</strong><p>The evidence shows how retrieval can fail. It cannot tell us how common each failure is across all users.</p></article></div><footer><span><strong>{system.bySource.length}</strong> source types represented</span><span><strong>{corpus.verified_count}/{corpus.evidence_count}</strong> stories human checked</span><span><strong>{system.publicExcluded}</strong> preserved records explicitly excluded</span></footer></section><div className="source-cards-v3">{system.sourceCoverage.sources.map((source) => { const description=sourceMethods[source.sourceKind]; return <article className={source.admittedEpisodes>0?"included":""} key={source.sourceKind}><div className="source-head-v3"><span className={source.status === "attempted"?"attempted":""}>{source.status === "attempted"?"Attempted":"Not connected"}</span><strong>{description?.name ?? friendlyLabel(source.sourceKind)}</strong></div><p>{description?.note}</p><div className="source-counts-v3"><div><strong>{source.uniqueCandidates.toLocaleString()}</strong><span>unique items screened</span></div><div><strong>{source.admittedEpisodes}</strong><span>stories included</span></div></div><footer><span>{description?.method}</span><strong>{description?.api}</strong></footer></article>; })}</div><div className="integrity-v3"><div><span>✓</span><p><strong>{corpus.verified_count} of {corpus.evidence_count} stories are human checked.</strong> Raw source, AI interpretation, and audit stay separate.</p></div><div><span>−</span><p><strong>{system.publicExcluded} preserved public records and {system.simulatedExcluded} illustrative records are explicitly excluded.</strong> Other screened items never passed into evidence.</p></div><div><span>!</span><p><strong>Large review volume is not relevant evidence by itself.</strong> The App Store supplied many candidates but no qualifying story.</p></div></div></>}
        ...
        {libraryView === "survey" && <><section className="survey-method-v3"><div><span className="section-label-v3">Anonymous primary research</span><h2>{system.primary.study.title}</h2><p>{system.primary.study.sampleType}. Responses are analyzed separately from public evidence, and the raw free-text answers remain private.</p></div><a className="secondary-v3" href={system.primary.study.instrumentUrl} target="_blank" rel="noreferrer">Open survey instrument ↗</a><div className="survey-method-stats-v3"><span><strong>{system.primary.study.responseCount}</strong> responses</span><span><strong>{system.primary.study.currentUserCount}</strong> current users</span><span><strong>{system.primary.study.recentEpisodeCount}</strong> recent attempts</span><span><strong>{system.primary.study.effortOrFailureCount}</strong> effort or failure</span></div><small>{system.primary.study.privacyNote} {system.primary.study.caveat}</small></section><div className="survey-episode-grid-v3">{system.primary.episodes.map((episode)=><article key={episode.id}><div className="survey-episode-top-v3"><span>{episode.id}</span><strong>{episode.targetType}</strong></div><h2>{episode.targetSummary ?? "Target not described"}</h2><div className="tags-v3">{episode.remembered.map((item)=><span key={`r-${item}`}>Remembered · {item}</span>)}{episode.forgotten.map((item)=><span className="forgotten" key={`f-${item}`}>Forgotten · {item}</span>)}</div><dl><div><dt>Started with</dt><dd>{episode.firstMethod}</dd></div><div><dt>First result</dt><dd>{episode.firstResult}</dd></div><div><dt>Final outcome</dt><dd>{episode.finalOutcome}</dd></div></dl>{episode.exactQuery&&<blockquote>Reported query: “{episode.exactQuery}”</blockquote>}<footer><span>{episode.qualityFlags.length ? "⚠ Incomplete or inconsistent fields" : "✓ Complete response"}</span></footer></article>)}</div></>}
      </section>}
    </main>
